...
<%
    ...
    private renderFieldForProperty(p, owningClass, prefix = "") {
        ...
        if (display) { 
%>
        <div class="col-xs-8 form-group \${hasErrors(bean: ${propertyName}, field: '${prefix}${p.name}', 'error')}">
            <label for="${prefix}${p.name}">
                <g:message code="${domainClass.propertyName}.${prefix}${p.name}.label" default="${p.naturalName}" />
                <% if (required) { %><span class="required-indicator">*</span><% } %>
            </label>
            ${renderEditor(p)}
            <% if (required) { %><span class="glyphicon glyphicon-exclamation-sign form-control-feedback" title="" data-message-required="\${message (code: "default.field.required")}" data-message-required="\${message (code: "default.field.required")}" aria-hidden="true"></span><% } %>
        </div>
<%      }   
    } 
%>
